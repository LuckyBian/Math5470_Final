...
if not os.path.exists(DATA_PATH):
    # Fallback to CWD or standard paths if needed
...
def prepare_data():
    ...
    print(f"主表特征训练集大小: {train.shape}")
    print(f"主表特征测试集大小: {test.shape}")
...
def main():
    ...
    core_features_count = len([col for col in train.columns if col not in ['TARGET', 'SK_ID_CURR']])
    print(f"当前使用的特征数量（仅主表特征）: {core_features_count}")
    
    # 训练模型
    model, features, feature_importance, oof_score = train_model_with_kfold(train)
    log_ablation_result('No_Aggregations', oof_score)
