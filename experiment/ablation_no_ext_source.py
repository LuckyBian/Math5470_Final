...
RESULTS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ablation_results.csv')
...
def process_previous_application_advanced():
    """处理 Previous Application 数据 - 增强版"""
    print("正在处理 Previous Application 数据（增强版）...")
...
def process_installments_advanced():
    """处理 Installments Payments 数据 - 增强版"""
    print("正在处理 Installments Payments 数据（增强版）...")
...
def process_credit_card_advanced():
    """处理 Credit Card Balance 数据 - 增强版"""
    print("正在处理 Credit Card Balance 数据（增强版）...")
...
def process_pos_cash_advanced():
    """处理 POS Cash Balance 数据 - 增强版"""
    print("正在处理 POS Cash Balance 数据（增强版）...")
...
def remove_ext_source_features(train, test):
    """移除所有包含 EXT_SOURCE 的特征"""
    ext_cols = sorted({
        col for col in train.columns.union(test.columns)
        if 'EXT_SOURCE' in col
    })
    
    if not ext_cols:
        print("未找到需要移除的 EXT_SOURCE 特征。")
        return train, test
    
    train = train.drop(columns=[col for col in ext_cols if col in train.columns])
    test = test.drop(columns=[col for col in ext_cols if col in test.columns])
    
    print(f"移除了 {len(ext_cols)} 个 EXT_SOURCE 相关特征:")
    print(", ".join(ext_cols))
    
    return train, test


def log_ablation_result(experiment_name, auc_score):
    """记录实验结果到 CSV（追加写入）"""
    header_needed = not os.path.exists(RESULTS_FILE)
    with open(RESULTS_FILE, 'a', encoding='utf-8') as f:
        if header_needed:
            f.write('Experiment,AUC\n')
        f.write(f'{experiment_name},{auc_score:.6f}\n')
    print(f"实验结果已写入 {RESULTS_FILE}")
...
def train_model_with_kfold(train):
    ...
    print(f"\n{'='*60}")
    print(f"总体 Out-of-Fold AUC: {oof_score:.6f}")
    print(f"CV 平均 AUC: {np.mean(cv_scores):.6f} (+/- {np.std(cv_scores):.6f})")
    print(f"{'='*60}")
    ...
    # 训练最终模型（使用全部数据，使用平均最佳迭代数）
    print("\n训练最终模型...")
    avg_best_iteration = NUM_BOOST_ROUNDS
    ...
    return final_model, features, feature_importance, oof_score
...
def make_predictions(model, test, features):
    ...
    print("预测结果已保存到: submission_advanced.csv")
...
def main():
    """主函数"""
    print("=" * 80)
    print("Home Credit Default Risk - Advanced Model with Feature Engineering")
    print("=" * 80)
    
    # 准备数据
    train, test = prepare_data()
    train, test = remove_ext_source_features(train, test)
    
    # 训练模型
    model, features, feature_importance, oof_score = train_model_with_kfold(train)
    log_ablation_result('No_Ext_Source', oof_score)
